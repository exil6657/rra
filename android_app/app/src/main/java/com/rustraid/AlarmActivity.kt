...
class AlarmActivity:ComponentActivity(){
 override fun onCreate(state:Bundle?){super.onCreate(state);val silent=intent.getBooleanExtra(AlarmService.EXTRA_SILENT,false);setShowWhenLocked(true);setTurnScreenOn(true);window.addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);setContent{RustRaidTheme{val acknowledge={FirebaseRepository().acknowledge();stopService(AlarmService.intent(this));finish()};if(silent)SilentAlertScreen(acknowledge) else AlarmScreen(acknowledge)}}}
}
