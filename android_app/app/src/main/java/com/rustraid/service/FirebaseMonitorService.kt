...
class FirebaseMonitorService:Service(){
 ...
 private fun startListeners(){if(alarmListener!=null||laptopId.isBlank()||phoneId.isBlank())return;val base=FirebaseDatabase.getInstance().getReference("laptops").child(laptopId);profileListener=object:ValueEventListener{override fun onDataChange(s:DataSnapshot){profile=s};override fun onCancelled(e:DatabaseError){}};base.child("phones").child(phoneId).child("profile").addValueEventListener(profileListener!!);alarmListener=object:ValueEventListener{override fun onDataChange(s:DataSnapshot){val active=s.child("alarm_active").getValue(Boolean::class.java)?:false;val target=s.child("device_target").getValue(String::class.java)?:"both";val p=profile;if(active&&(target=="phone"||target=="both"))ContextCompat.startForegroundService(this@FirebaseMonitorService,AlarmService.intent(this@FirebaseMonitorService,s.child("force_silent").getValue(Boolean::class.java)?:false,p?.child("vibration")?.getValue(Boolean::class.java)?:true,p?.child("screen_flash")?.getValue(Boolean::class.java)?:true,laptopId,(p?.child("auto_silence_minutes")?.getValue(Long::class.java)?:5L).toInt(),p?.child("volume_override")?.getValue(Boolean::class.java)?:true,p?.child("sound_preset")?.getValue(String::class.java)?:"defcon1")) else stopService(AlarmService.intent(this@FirebaseMonitorService,laptopId=laptopId))};override fun onCancelled(e:DatabaseError){}};base.child("raid_alarm").addValueEventListener(alarmListener!!);heartbeatHandler.removeCallbacks(heartbeat);heartbeatHandler.post(heartbeat)}
