...
class AlarmService:Service(){
 ...
 override fun onStartCommand(i:Intent?,f:Int,id:Int):Int{
  ...
  active=true;laptopId=i?.getStringExtra(EXTRA_LAPTOP_ID)?:"";val silent=i?.getBooleanExtra(EXTRA_SILENT,false)?:false;val vibration=i?.getBooleanExtra(EXTRA_VIBRATION,true)?:true;val flash=i?.getBooleanExtra(EXTRA_FLASH,true)?:true;val autoMinutes=(i?.getIntExtra(EXTRA_AUTO_SILENCE,5)?:5).coerceIn(1,15);val volumeOverride=i?.getBooleanExtra(EXTRA_VOLUME_OVERRIDE,true)?:true;val manager=getSystemService(NotificationManager::class.java)
 ...
 companion object{const val EXTRA_SILENT="silent";const val EXTRA_VIBRATION="vibration";const val EXTRA_FLASH="flash";const val EXTRA_LAPTOP_ID="laptop_id";const val EXTRA_AUTO_SILENCE="auto_silence_minutes";const val EXTRA_VOLUME_OVERRIDE="volume_override";fun intent(c:Context,silent:Boolean=false,vibration:Boolean=true,flash:Boolean=true,laptopId:String="",autoSilenceMinutes:Int=5,volumeOverride:Boolean=true)=Intent(c,AlarmService::class.java).putExtra(EXTRA_SILENT,silent).putExtra(EXTRA_VIBRATION,vibration).putExtra(EXTRA_FLASH,flash).putExtra(EXTRA_LAPTOP_ID,laptopId).putExtra(EXTRA_AUTO_SILENCE,autoSilenceMinutes).putExtra(EXTRA_VOLUME_OVERRIDE,volumeOverride)}
}
