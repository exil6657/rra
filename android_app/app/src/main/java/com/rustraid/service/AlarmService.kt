...
class AlarmService:Service(){
 ...
 override fun onStartCommand(i:Intent?,f:Int,id:Int):Int{
  ...
  active=true;val silent=i?.getBooleanExtra(EXTRA_SILENT,false)?:false;val vibration=i?.getBooleanExtra(EXTRA_VIBRATION,true)?:true;val flash=i?.getBooleanExtra(EXTRA_FLASH,true)?:true;val manager=getSystemService(NotificationManager::class.java)
  ...
  val alarmIntent=PendingIntent.getActivity(this,1,Intent(this,AlarmActivity::class.java).putExtra(EXTRA_SILENT,silent).putExtra(EXTRA_FLASH,flash).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TOP),PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
 ...
 companion object{const val EXTRA_SILENT="silent";const val EXTRA_VIBRATION="vibration";const val EXTRA_FLASH="flash";fun intent(c:Context,silent:Boolean=false,vibration:Boolean=true,flash:Boolean=true)=Intent(c,AlarmService::class.java).putExtra(EXTRA_SILENT,silent).putExtra(EXTRA_VIBRATION,vibration).putExtra(EXTRA_FLASH,flash)}
}
