...
class AlarmService:Service(){
 ...
 override fun onStartCommand(i:Intent?,f:Int,id:Int):Int{
  ...
  active=true;val laptopId=i?.getStringExtra(EXTRA_LAPTOP_ID)?:"";val silent=i?.getBooleanExtra(EXTRA_SILENT,false)?:false;val vibration=i?.getBooleanExtra(EXTRA_VIBRATION,true)?:true;val flash=i?.getBooleanExtra(EXTRA_FLASH,true)?:true;val manager=getSystemService(NotificationManager::class.java)
  ...
  val ackIntent=PendingIntent.getBroadcast(this,2,Intent(this,AcknowledgeReceiver::class.java).putExtra(EXTRA_LAPTOP_ID,laptopId),PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
 ...
 companion object{const val EXTRA_SILENT="silent";const val EXTRA_VIBRATION="vibration";const val EXTRA_FLASH="flash";const val EXTRA_LAPTOP_ID="laptop_id";fun intent(c:Context,silent:Boolean=false,vibration:Boolean=true,flash:Boolean=true,laptopId:String="")=Intent(c,AlarmService::class.java).putExtra(EXTRA_SILENT,silent).putExtra(EXTRA_VIBRATION,vibration).putExtra(EXTRA_FLASH,flash).putExtra(EXTRA_LAPTOP_ID,laptopId)}
}
