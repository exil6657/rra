...
class AlarmService:Service(){
 ...
 companion object{const val EXTRA_SILENT="silent";const val EXTRA_VIBRATION="vibration";const val EXTRA_FLASH="flash";const val EXTRA_LAPTOP_ID="laptop_id";const val EXTRA_AUTO_SILENCE="auto_silence_minutes";fun intent(c:Context,silent:Boolean=false,vibration:Boolean=true,flash:Boolean=true,laptopId:String="",autoSilenceMinutes:Int=5)=Intent(c,AlarmService::class.java).putExtra(EXTRA_SILENT,silent).putExtra(EXTRA_VIBRATION,vibration).putExtra(EXTRA_FLASH,flash).putExtra(EXTRA_LAPTOP_ID,laptopId).putExtra(EXTRA_AUTO_SILENCE,autoSilenceMinutes)}
}
