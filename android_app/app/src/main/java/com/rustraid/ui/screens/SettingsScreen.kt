...
@Composable fun SettingsScreen(settings:AppSettings,onUpdate:(Map<String,Any>)->Unit,onBatteryExemption:()->Unit,onFullScreenPermission:()->Unit,onDndAccess:()->Unit,onNotifications:()->Unit,onUnlink:()->Unit,unlinkStatus:String,customSoundUri:String,onCustomSound:(String)->Unit){
 ...
 Column(Modifier.padding(20.dp).verticalScroll(rememberScrollState()),verticalArrangement=Arrangement.spacedBy(14.dp)){
  ...
  SettingSwitch("Screen flash",settings.screenFlash){onUpdate(mapOf("screen_flash" to it))};Text("Phone alarm sound",style=MaterialTheme.typography.titleMedium);Row(horizontalArrangement=Arrangement.spacedBy(8.dp)){FilterChip(selected=settings.phoneSoundPreset=="defcon1",onClick={onUpdate(mapOf("phone_sound_preset" to "defcon1"))},label={Text("DEFCON")});FilterChip(selected=settings.phoneSoundPreset=="tactical",onClick={onUpdate(mapOf("phone_sound_preset" to "tactical"))},label={Text("Tactical")});FilterChip(selected=settings.phoneSoundPreset=="custom",onClick={onUpdate(mapOf("phone_sound_preset" to "custom"))},label={Text("Custom")})};if(settings.phoneSoundPreset=="custom"){Button(onClick={customPicker.launch(arrayOf("audio/*"))}){Text("Choose custom audio")};Text(if(customSoundUri.isBlank())"No custom audio selected; DEFCON will be used as fallback." else "Custom audio selected on this phone.",style=MaterialTheme.typography.bodySmall)};SettingSwitch("Vibration",settings.vibration){onUpdate(mapOf("vibration" to it))};SettingSwitch("Force maximum alarm volume",settings.volumeOverride){onUpdate(mapOf("volume_override" to it))};SettingSwitch("Quiet hours",settings.quietHoursEnabled){onUpdate(mapOf("quiet_hours_enabled" to it))}
  ...
  SettingSwitch("AMOLED black theme",settings.amoledBlack){onUpdate(mapOf("amoled_black" to it))};SettingSwitch("Keep screen on during cooldown",settings.keepScreenOnDuringCooldown){onUpdate(mapOf("keep_screen_on_during_cooldown" to it))}
