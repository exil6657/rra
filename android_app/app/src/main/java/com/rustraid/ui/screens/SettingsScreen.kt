...
@Composable fun SettingsScreen(settings:AppSettings,onUpdate:(Map<String,Any>)->Unit,onBatteryExemption:()->Unit){
 Column(Modifier.padding(20.dp).verticalScroll(rememberScrollState()),verticalArrangement=Arrangement.spacedBy(14.dp)){
  ...
  Text("Alarm target",style=MaterialTheme.typography.titleMedium)
  Row(horizontalArrangement=Arrangement.spacedBy(8.dp)){TargetChip("Laptop", "laptop",settings.deviceTarget,onUpdate);TargetChip("Phone", "phone",settings.deviceTarget,onUpdate);TargetChip("Both", "both",settings.deviceTarget,onUpdate)}
  Text("Phone alert mode",style=MaterialTheme.typography.titleMedium)
