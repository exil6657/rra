...
class MainActivity:ComponentActivity(){
 ...
 override fun onCreate(b:Bundle?){super.onCreate(b);if(Build.VERSION.SDK_INT>=33&&ContextCompat.checkSelfPermission(this,Manifest.permission.POST_NOTIFICATIONS)!=PackageManager.PERMISSION_GRANTED)ActivityCompat.requestPermissions(this,arrayOf(Manifest.permission.POST_NOTIFICATIONS),10);FirebaseSession.ensureAuthenticated({},{});setContent{val pairingManager=remember{PairingManager(applicationContext)};val pairing by pairingManager.info.collectAsState(initial=PairingInfo());if(!pairing.linked)RustRaidTheme{PairingFlow(pairing,pairingManager)}else LinkedApp(pairing,pairingManager)}}
 @Composable private fun PairingFlow(pairing:PairingInfo,manager:PairingManager){
  ...
  PairingScreen(pairing.laptopId.isNotBlank()&&pairing.pendingSecret.isNotBlank(),message,{code,name->lifecycleScope.launch{manager.request(code,name){result->message=if(result.isSuccess)"Pair request sent. Accept it on the laptop." else result.exceptionOrNull()?.message?:"Pairing failed."}}},{lifecycleScope.launch{manager.cancelPending(pairing);message="Pairing request cancelled."}})
 }
