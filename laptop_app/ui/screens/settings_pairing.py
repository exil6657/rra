...
class PairingSettings(QWidget):
 ...
 def refresh(self):
  ...
  try:
   import qrcode
   path=Path(__file__).resolve().parents[2]/'assets'/'pairing_qr.png';qrcode.make(code).save(path);self.qr.setPixmap(QPixmap(str(path)).scaled(220,220,Qt.AspectRatioMode.KeepAspectRatio,Qt.TransformationMode.SmoothTransformation))
