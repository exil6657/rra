...
class Dashboard(QWidget):
    ...
    def __init__(self,config):
        ...
        grid.addWidget(self.card('Quick Settings',quick),1,0); self.firebase_health='unavailable';self.phone_health='unknown';self.summary=QLabel();self.refresh_summary('monitoring',0); grid.addWidget(self.card('Current Status Summary',[self.summary]),1,1)
    ...
    def refresh_summary(self,state='monitoring',seconds=0):
        cooldown=f'{seconds//3600:02}:{seconds%3600//60:02}:{seconds%60:02}' if state=='cooldown' else 'inactive';quiet='enabled' if self.config['cooldown'].get('quiet_hours_enabled') else 'off';self.summary.setText(f'Cooldown: {cooldown}\nQuiet hours: {quiet}\nFirebase: {self.firebase_health}\nPhone: {self.phone_health}')
