...
class Dashboard(QWidget):
    ...
    def __init__(self,config):
        super().__init__(); self.config=config; l=QVBoxLayout(self); self.banner=QLabel('● MONITORING // READY TO WATCH SELECTED REGION'); self.banner.setObjectName('pageTitle'); self.banner.setStyleSheet('background:#161020;border:1px solid #A855F7;padding:18px;border-radius:10px'); l.addWidget(self.banner)
    ...
    def card(self,title,widgets):
        f=QFrame(); f.setProperty('role','commandCard'); v=QVBoxLayout(f); h=QLabel(title); h.setStyleSheet('font-weight:800;font-size:17px'); v.addWidget(h)
    ...
    def set_state(self,state,seconds=0):
        colors={'monitoring':'#161020','raid':'#650B0B','cooldown':'#593400','disconnected':'#181820'}; texts={'monitoring':'● MONITORING // PC LINK ONLINE','raid':'⚠ RAID DETECTED // ACKNOWLEDGE','cooldown':f'◷ COOLDOWN // {seconds//3600:02}:{seconds%3600//60:02}:{seconds%60:02}','disconnected':'◌ OFFLINE // RECONNECTING'}
