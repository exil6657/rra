...
class DiscordSettings(QWidget):
    ...
    def commit(self,template_path=None):
        current=self.config['screen_monitor'];self.save('screen_monitor',{'enabled':self.enabled.isChecked(),'region':self.region,'trigger_text':self.text.text().strip(),'template_path':template_path if template_path is not None else current.get('template_path',''),'poll_interval_ms':self.interval.value(),'confirm_frames':self.confirm.value(),'template_threshold':self.threshold.value()})
