...
def main():
 ...
 def raid(data):
  ...
  if not remote:
   firebase.write_alarm({'alarm_active':True,'acknowledged':False,'triggered_at':triggered_at,'channel_name':'selected screen region','device_target':target,'alert_mode':phone_mode,'force_silent':local_mode=='silent','phone_vibration':phone_vibration,'phone_screen_flash':phone_flash,'phone_volume_override':phone_volume_override,'phone_sound_preset':phone_sound_preset,'auto_silence_minutes':auto_silence})
   if target in ('phone','both'): firebase.send_phone_alarm(triggered_at,'Visual alert detected',force_phone_mode)
 ...
 def update_pairing_health(details):
  phones=details.get('phones',{});window.dashboard.set_health(phone=(f'{len(phones)} phone(s) linked; waiting for heartbeat' if phones else 'not paired'))
 def update_phone_roster(phones):
  connected=sum(1 for phone in phones.values() if phone.get('heartbeat'));window.dashboard.set_health(phone=f'{connected}/{len(phones)} phone(s) reporting' if phones else 'not paired')
 monitor.detected.connect(lambda data:raid({'author':'screen monitor ('+data['reason']+')'}));monitor.state_changed.connect(lambda state:display_state('monitoring' if state=='monitoring' else 'disconnected'));monitor.error.connect(lambda message:event('Screen monitor error: '+message));cooldown.changed.connect(display_state);cooldown.expired.connect(lambda:(display_state('monitoring'),event('Monitoring resumed')));cooldown.auto_silence.connect(lambda:acknowledge('auto-silence'));firebase.remote_state.connect(sync_remote);firebase.remote_settings.connect(apply_remote_settings);firebase.remote_meta.connect(update_phone_health);firebase.remote_phones.connect(update_phone_roster);firebase.pairing_changed.connect(update_pairing_health);firebase.connected.connect(lambda ok:window.dashboard.set_health(firebase='connected ✓' if ok else 'unavailable'));firebase.remote_acknowledged.connect(lambda source:acknowledge('remote:'+str(source or 'device')));window.dashboard.set_health(firebase='connected ✓' if firebase.db else 'unavailable',phone=(f"{len(c['pairing'].get('phones',{}))} phone(s) linked; waiting for heartbeat" if c['pairing'].get('phones') else 'not paired'))
