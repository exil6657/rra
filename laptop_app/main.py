...
def main():
 ...
 def apply_remote_settings(shared):
  ...
  for remote,local in (('vibration','phone_vibration'),('screen_flash','phone_screen_flash')):
 ...
 def update_phone_health(meta):
  ...
  if not seen: window.dashboard.set_health(phone='not seen');return
  try:
 ...
 monitor.detected.connect(lambda data:raid({'author':'screen monitor ('+data['reason']+')'}));monitor.state_changed.connect(lambda state:display_state('monitoring' if state=='monitoring' else 'disconnected'));monitor.error.connect(lambda message:event('Screen monitor error: '+message));cooldown.changed.connect(display_state);cooldown.expired.connect(lambda:(display_state('monitoring'),event('Monitoring resumed')));cooldown.auto_silence.connect(lambda:acknowledge('auto-silence'));firebase.remote_state.connect(sync_remote);firebase.remote_settings.connect(apply_remote_settings);firebase.remote_meta.connect(update_phone_health);firebase.connected.connect(lambda ok:window.dashboard.set_health(firebase='connected ✓' if ok else 'unavailable'));firebase.remote_acknowledged.connect(lambda source:acknowledge('remote:'+str(source or 'device')));window.dashboard.set_health(firebase='connected ✓' if firebase.db else 'unavailable')
